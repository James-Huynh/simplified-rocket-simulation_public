...
public class CalculsTrajectoire {
	...
	private final static double DELTA_T = 0.01;
	private final static double ANGLE_TOTAL = 360.0;
	/**
	 * Méthode qui calcule et retourne un ArrayList de points représentant la trajectoire
	 * @param masse la masse de l'objet
	 * @param position la position de l'objet
	 * @param vitesse la vitesse de l'objet
	 * @param nCalculsParPoint le nombre de calculs physiques entre chaque point
	 * @return un ArrayList de points représentant la trajectoire
	 */
	public static ArrayList<Point.Double> calculerTrajectoire(double masse, Vecteur position, Vecteur vitesse, int nCalculsParPoint) {
		...
		do {
			...
			for (int k = 0; k < nCalculsParPoint; k++) {
				...
				MoteurPhysiqueOrbite.unCalculOrbite(sommeForces, positionLocale, vitesseLocale, acceleration, masse, DELTA_T);
			}
		...
		} while(angleTotal < (ANGLE_TOTAL));
		
		return listePoints;
	}
	/**
	 * Méthode qui crée et retourne un Path2D représentant la trajectoire
	 * @param listePoints un ArrayList de points représentant la trajectoire
	 * @return un Path2D représentant la trajectoire
	 */
	...
	private static double angleEntreDeuxVecteurs(Vecteur v1, Vecteur v2) {
		return Math.toDegrees(Math.acos(dotProduct(v1, v2)/(v1.module() * v2.module())));
	}
}
