...
public class ZoneDessinOrbitale extends JPanel implements Runnable {
	...
	private final String TXT_ECHELLE = ("500km / intervalle, "  + "N.B. La fusée n'est pas à l'échelle. Elle a été grossie de " + FACTEUR_IMAGE_FUSEE + " fois."),
			TXT_MESSAGE_FIN = "Ici tour de contrôle, échec de la mission.";
	...
	public void paintComponent(Graphics g) {
		...
		g2d.setColor(couleurHalo);//Dessiner le halo derrière la capsule
		creerHalo();
	...
	/**
	 * Méthode pour transformer l'image initiale de la Terre pour qu'elle ait les bonnes dimensions reflétant la realité 
	...
	/**
	 * Méthode pour transformer l'image initiale de l'espace pour qu'elle soit bien dimensionnée dans la fenêtre
	 * @param mat La matrice pour convertir des objets du monde réel vers l'espace en pixel
	 */
	 ...
	 * @param touche La touche directionnel appuyée
	 ...
	 */
	public void reinitialiserValeursOrbitales() { //appelé quand bouton pour revenir à l'assemblage chosi sinon capsule reset d'avance
		position = new Vecteur (0.0, RAYON_TERRE + 100000.0);
		vitesse =  new Vecteur (0.0, 0.0);
		acceleration = new Vecteur (0.0, 0.0);
		vForceGrav = new Vecteur (0.0, 0.0);
	...
	private void initialiserImages() {
		URL urlImageTerre = ZoneDessinOrbitale.class.getResource(nomImageTerre);
		URL urlImageEspace = ZoneDessinOrbitale.class.getResource(nomImageEspace);
	...
	/**
	 * Méthode pour créer le halo coloré derrière la capsule pour améliorer la visibilité
	 */
	 ...
	 * @param acceleration L'accélération de la fusée
	 */
